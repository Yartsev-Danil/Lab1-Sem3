...
{
	if (!items || count <= 0)
		throw logic_error("no items or size error");
	this->count = count;
...
ArraySequence<T> ::ArraySequence(const ArraySequence<T>& other)
{
	if (!other.count || !other.data)
		throw logic_error("no data or size error");
...
void ArraySequence<T> ::Remove(const T value)
{
	...
	if (index == -1)
		throw logic_error("there is not value of argument in the array");
};
